...
use crate::handlers::markups::callbacks::module::{module_option_handler, module_select_handler, module_toggle_handler, settings_back_handler, settings_set_handler};
...
pub(crate) async fn callback_query_handlers(bot: Bot, q: CallbackQuery) -> Result<(), MyError> {
    ...
    // tokio::spawn(async move {
        ...
        if data.starts_with("settings_set:") {
            settings_set_handler(bot, qq).await?
            ...
            bot.answer_callback_query(qq.id).await?;
        }
    // });
