...
use crate::handlers::commands::translate::{create_language_keyboard, normalize_language_code, SUPPORTED_LANGUAGES};
...
async fn handle_language_selection(
    ...
    let original_message = match message.reply_to_message() {
        Some(msg) => msg,
        None => {
            bot.edit_message_text(message.chat.id, message.id, "Ошибка: не удалось найти исходное сообщение. Попробуйте снова.")
                .await?;
            return Ok(());
        }
    };
    ...
    let text_to_translate = match original_message.text() {
        Some(text) => text,
        None => {
            bot.edit_message_text(message.chat.id, message.id, "В исходном сообщении нет текста для перевода.")
                .await?;
            return Ok(());
        }
    };
